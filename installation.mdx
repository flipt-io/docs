...
Docker installation is required on the host, see the
[official installation docs](https://docs.docker.com/install/).
...
<Note>
  Flipt runs without root in the Docker container as of
...
<Note>
  The Flipt Helm chart is still a work in progress. Any issues or suggestions on
  how to improve it are welcome in the [chart
  repository](https://github.com/flipt-io/helm-charts).
</Note>
...
Flipt is built for the following architectures:
...
The [Docker image](https://hub.docker.com/r/flipt/flipt/tags) for Flipt is also multi-arch and supports both **amd64** and **arm64** architectures.
