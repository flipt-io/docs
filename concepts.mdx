...
- **Boolean** which allows you to return a boolean value for a given flag given a set of rollout rules.
...
Rollouts allow you to configure boolean flags to return `true` or `false` for a given flag given a set of rollout rules.
...
<Note>
  The `default` rollout is a special rollout that's always evaluated last. This
  allows you to return a default value if no other rollouts match. It's value is
  directly tied to the `enabled` value of the flag.
</Note>
