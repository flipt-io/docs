...
We are actively designing and developing client-side implementations to leverage this method seemlessly.
...
This allows services deployed into the same Kubernetes cluster as Flipt to automatically gain authenticated access to the Flipt API without additional management of static cient tokens.

![Kubernetes Authentication Flow](/images/authentication/kubernetes.svg)

When enabled (to enable this method see our [Configuration: Method Kubernetes](/configuration/authentication#method-kubernetes) documentation) a service deployed within Kubernetes can read their service account token from disk and invoke the verify service account operation on the API.
Given the service account is deemed valid for the surrounding cluster this operation will return a valid client token with a matching expiration as the service account.
...
<Tip>
Kubernetes refreshes service account tokens locally.
All that is required is to read the token from disk again to get the new one.
</Tip>
