...
Flipt v2 is designed to be self-hosted and gives you complete control over your feature flag infrastructure.
...
Flipt v2 introduces the concept of environments, which are an additional layer above Flipt v1's concept of [Namespaces](/v2/concepts#namespace).
...
- **AWS Secrets Manager** - Retrieve secrets using standard AWS credentials
- **GCP Secret Manager** - Retrieve secrets with Application Default Credentials or service account keys
- **Azure Key Vault** - Retrieve secrets using Azure identity credentials
