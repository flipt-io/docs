...
For guidance on setting up Git repositories with existing feature files, see our [Git Repository Initialization guide](/v2/guides/operations/environments/git-repository-initialization).
...
The `fetch_policy` configuration option controls how Flipt handles Git fetch operations when syncing with remote repositories. This option allows you to specify the merge strategy used when pulling changes from the remote repository.

Available fetch policies:

- `ff_only` (default): Only allow fast-forward merges. This is the safest option as it will fail if there are conflicts that require a merge commit.
- `merge`: Allow merge commits when fast-forward is not possible. This will create merge commits to resolve conflicts.
- `rebase`: Rebase local changes on top of remote changes. This maintains a linear history but rewrites commit history.
...
storage:
  production:
    ...
    branch: "main"
    poll_interval: "30s"
    fetch_policy: "ff_only"  # Use fast-forward only strategy
    credentials: "github"
    backend:
      type: local
      path: "./data"
...
<Warning>
  Using `rebase` or `merge` fetch policies may result in unexpected behavior if multiple Flipt instances are writing to the same repository branch. The `ff_only` policy is recommended for production environments to prevent conflicts.
</Warning>
