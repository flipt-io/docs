...
Some OIDC providers require extra query parameters on the authorization request. For example, Auth0 can use an `audience` parameter to request API access, and some providers use parameters such as `domain_hint`, `connection`, or `idp` to route users to the correct identity provider.

Configure these values with the `authorize_parameters` map on the provider. Flipt appends each key-value pair to the OIDC authorize URL during login.
