...
Before upgrading your Flipt instances to a new version, you need to run the `flipt migrate` command against your database.
...
<Warning>
We **strongly recommend** that you perform a backup of your Flipt database before doing any migrations.
While we strive to ensure that Flipt is error free and production ready, we cannot guarantee perfection.
</Warning>

First, run `flipt -v` to see the version you have installed locally.
...
We design our migrations to be forward compatible with older Flipt server versions.
So it is safe for you to keep running Flipt while the migrations are run and after they have finished.

## Downgrade Process

As mentioned, we recommend you backup your database before performing any upgrades.
Flipt does not currently come with any built-in downgrade tooling.
