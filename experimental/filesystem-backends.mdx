...
It will also use the same default strategy when the index is not supplied (e.g. files name `features.yml` or `*.features.yml`).
...
In addition to these Flipt configuraton parameters, credentials will also be required for Flipt to authenticate with the target object store.
...
Meaning, any file named `features.yaml`, `features.yml` or with either extension `.features.yaml` or `.features.yml` is considered recursively from the root of your target.
