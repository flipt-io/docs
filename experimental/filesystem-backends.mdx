...
<Note>
GitHub PAT (Personal Access Tokens) should be supplied using `basic` authentication as the password.
...
<Info>
Flipt flag state file format has been taken directly from Flipts existing [import and export](/configuration/storage#import-export) flag state format.
You can run `flipt export` and add / commit the result to a directory or git repository to get started quickly.
</Info>
...
<Tip>
By defining different namespaces in different directories, you can leverage features such as GitHub's Codeowners.
This gives you authorization mechanisms for managing contributions to Flipt state.
</Tip>
...
<Info>All the resources in Flipt are explained in more depth in the [Concepts](/concepts) documentation page.</Info>
...
Each segment has a `match_type` which signifies whether or not the *all* constraints have to match or *any* (at-least one) must match.
