...
Flipt Cloud only supports [Client-Side Evaluation](/v1/integration/client) currently, so these API endpoints are not available if you are using Flipt Cloud.

See the [Flipt Cloud Architecture](/v1/cloud/architecture) section for more information.
