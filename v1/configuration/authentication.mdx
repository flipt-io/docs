...
See the [Authentication: Overview](/v1/authentication/overview) documentation for more details on Flipt's API authentication handling.
...
Once enabled, static tokens can be created via the [CreateToken](/v1/reference/authentication/create-token) operation in the API.

Further explanation for using this method can be found in the [Authentication: Static Token](/v1/authentication/methods#static-token) documentation.
...
<Tip>
  Read our [Login with
  Google](/v1/guides/operation/authentication/login-with-google) guide for a
  more in-depth walk-through setting up an OIDC provider.
</Tip>
...
To enable this, you must set the [`use_pkce`](/v1/configuration/overview#authentication-methods-oidc) property to `true` for each provider you would like to leverage PKCE with.

#### Example: OIDC With Google

<Tip>
  Checkout our [Login with
  Google](/v1/guides/operation/authentication/login-with-google) guide for an
...
<Tip>
  Read our [Login with
  Github](/v1/guides/operation/authentication/login-with-github) guide for a
...
!["GitHub Login"](/v1/images/guides/login-with-github/login-with-github.png)
