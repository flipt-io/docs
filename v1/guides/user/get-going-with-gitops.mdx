...
![Flipt instance showing flag in disabled state](/v1/images/guides/get-going-with-gitops/flag-defined-locally.png)
...
<Info>
  ...
  Backends](/v1/configuration/storage#declarative) to learn more about
...
This [segment](/v1/concepts#segments) definition matches any evaluation request where there exists a key `organization` on the context, with a value `internal`.
...
Finally, we add a [rollout rule](/v1/concepts#rollouts) to our boolean flag.
...
<Info>
  Head to [Concepts: Bucketing](/v1/concepts#bucketing) to learn how this
  mechanism is actually achieved.
</Info>
...
Checkout the [Configuration: Storage: Declarative](/v1/configuration/storage#declarative) for details on adjusting these intervals.
