...
Head to the [Configuration: Authentication](/v1/configuration/authentication) section to learn how to enable it.
...
See the [Configuration: Authentication Exclusions](/v1/configuration/authentication#exclusions) documentation for details.

## Client Tokens

Client tokens are the core credential required to authenticate a request.
Tokens themselves are acquired via [authentication methods](/v1/authentication/methods).

Flipt supports multiple authentication methods for acquiring credentials:

1. [Static Token](/v1/authentication/methods/#static-token)
1. [OIDC](/v1/authentication/methods/#oidc)
1. [GitHub](/v1/authentication/methods/#github)
1. [Kubernetes](/v1/authentication/methods/#kubernetes)

Once a `client token` has been acquired, it can be supplied via request metadata dependent on the protocol. Both HTTP and gRPC examples can be found on the [Using Client Tokens](/v1/authentication/using-tokens) page.
...
See the [Configuration: JWT Authentication](/v1/configuration/authentication#json-web-token) documentation for details.
