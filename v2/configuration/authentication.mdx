...
You can customize these mappings using the `claims_mapping` configuration option. This allows you to specify [JSON Pointer](https://datatracker.ietf.org/doc/html/rfc6901) expressions to extract user att
ributes from different locations in the JWT payload.
...
<Note>
  Only the predefined attribute names (`email`, `name`, `sub`, `picture`, `role`) are supported in claims mapping. Custom attribute names are not allowed to ensure compatibility with consuming code.
</Note>
...
<Note>
  Invalid JSON Pointer expressions or paths that don't exist in the JWT payload are silently ignored.
</Note>
