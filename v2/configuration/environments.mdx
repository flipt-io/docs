...
<Note>
  Source Control Management is a paid enterprise feature and require an
  enterprise license.
</Note>
...
Environments can be configured to use a single Source Control Management (SCM) provider such as GitHub, Gitea, GitLab, or Bitbucket.

<Warning>
  As of the alpha release, only GitHub and Gitea are supported, however we will be adding
  support for GitLab and Bitbucket soon.
</Warning>
