...
<Note>
  Source Control Management is a paid enterprise feature and requires an
  enterprise license.
</Note>
...
Environments can be configured to use a single Source Control Management (SCM) provider such as GitHub, GitLab, or Bitbucket.

<Warning>
  As of the alpha release, only GitHub is supported, however we will be adding
