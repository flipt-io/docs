...
- Log level to 'INFO'
- Local storage backend that serves flags from the local filesystem

<Note>
  This example uses local storage for simplicity. To configure Git repository
  synchronization, see the [Git Storage
  guide](/v2/guides/operations/environments/git-sync) for details on setting up
  remote Git sync.
</Note>
