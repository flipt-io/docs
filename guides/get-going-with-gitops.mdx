...
Our guide starts with an imaginary organization, with a single web application defined in Go and committed to a single GitHub repository.
...
For the examples sake, we assume the repository will be hosted on GitHub at https://github.com/organization/repository.git.
...
This will also return an identifier, only this time for the requesting users organization.
...
The name comes from the fact that the backend for Flipt is modelled around configuration files in a directory structure.
...
<Note>
You don't have to call your file `features.yml` and you can spread your flag definitions across multiple files.
Checkout our docs on [locating flag state](/configuration/storage#locating-flag-state) to learn more.
</Note>
...
Flipt will track the HEAD of this repositories `main` branch.
Changes will eventually propagate into our running instance of Flipt.

<Info>
Head to [Configuration: Storage: Filesystem Backends](/configuration/storage#filesystem) to learn more about configuring these backend types for Flipt.
</Info>
...
Now our application is deployed with our code change and is referencing our new flag, we can adjust the state via the configuration file and push the changes to Git.
...
Then we open the `features.yml` file and update definition with a new segment, and a rollout rule on our flag which returns `enable = true` when the request matches our new segment.
...
This segment definition matches any evaluation request where there exists a key `organization` on the context, with a value `internal`.
...
Once we've have an approval, we can merge the PR and the change goes live.
...
<Tip>
This is your opportunity to validate the change before exposing it to a wider audience.
Perhaps the algorithm is incorrect, maybe the order has reversed or has become unstable for some entries.
You can fix those changes now before targeting more users.
</Tip>

### Proportional Rollout

Once we're confident our change is working as expected, because we've have validated the change in production for `internal` users, we can start to roll it out to external users.

We could enable the flag for all users at once, but there is always a chance we've have missed something during manual validation.
Everyone has the best intentions, but things get missed.
So we're going to start slow and gradually enable it for percentages of our user-base.
This will give the change time to bake with your audience.
If anything is wrong, we've have minimized the effected users to a subset.
...
Note that we've have left our segment targeting rule intact.

This means our flag will remain enabled for internal users.
Rollouts rules are evaluated on each request in order.
...
<Info>
Head to [Concepts: Bucketing](/concepts#bucketing) to learn how this mechanism is actually achieved.
</Info>
...
At each commit, we could've fully recreate the configuration of our entire application, including the state of Flipt itself.

### Further Considerations

Now you have all the tools necessary to practice GitOps with your feature flags.

However, in production, you mileage may vary with the different backed types.
...
Beyond here, you may want to checkout the following to learn more about Flipt's features and operations:

- [Login with Google](/guides/login-with-google) for authenticating the UI
- [Deploy to Kubernetes](/guides/deploy-to-kubernetes) for deploying to Kubernetes
- [Configuration: Observability](/configuration/observability) for monitoring in production
