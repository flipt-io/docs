...
## Creating OAuth 2.0 Application on GitHub
...
This guide shows the basics of getting Flipt running with GitHub OAuth 2.0 authentication in a development setting using `ngrok`. Now that you know the basics, you can tailor the configuration pieces to fit your exact use cases. For instance, you would not use `ngrok` in a production setting, but rather a custom domain. If you have a custom domain, you can modify the `Authorization Callback URL` value on the GitHub OAuth application page, the `redirect_address`, and `domain` configuration values for the Flipt configuration.
