...
Your consent screen is the page you're navigated to when attemping to login via Google.
...
<Warning>
  You will have the option to create you OAuth application as `internal` or
  `external`. We recommened `internal` as that way only your internal Google
  workspace users can access Flipt.
</Warning>

Make sure to configure the consent screen as per Googles instructions and set the scopes accordingly.

### 2. Open [Google Console Credentials](https://console.cloud.google.com/apis/credentials)

You can simply navigate to the link in the title here of this section, or you
can also get there by going to the `API` section of your Google Cloud account and then select `Credentials`.
...
Now that we have an OAuth client configured in our Google Cloud account,
we can begin configuring Flipt to leverage it.

### 1. Add `google` provider to `config.yml`

Open your `config.yml` we created in the [beginning of the guide](#2-define-a-flipt-config-yml).
...
It is used by Flipt as the domain for storing authentication cookies.

Note that we have enabled the `oidc` method, and it has a section called `providers`.
...
There are a few touch points where you're going to want to update with your new domain.
Take for example, the domain `https://flipt.internal.dev`.
...
Sometimes for security purposes you dont want to be inserting your sensitive credentials (such as provider `client_secret` or your CSRF `key`) into a configuration file.
