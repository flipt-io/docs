...
description: Configuring Flipt to enable login in with Google via OIDC.
---
...
However, neither of these methods are appropriate for browser sessions and the UI.
...
- 🔑 Created an OAuth Client in your Google workspace
...
OIDC is an extension ontop of the OAuth standard for delegated authentication.
OAuth allows Flipt to delegate authentication onto an external provider of your choice.
...
It ensures that we can support a multitide of IdPs through a standard set of configuration parameters.
...
This is an optional step, however, we suggest you create yourself a blank directory in which to experiment with Flipt.
...
This is needed to ensure that Flipt enforces its APIs must be provided with a credential of some sort to gain access.
...
This particular command forwards your localhost port `8080` into the containers localhost post `8080`.
...
Once you visit Flipts UI, you should be greated by a message stating that there are no login providers configured.
...
As shown in the screenshot, we want enter the following values for the inputs:
...
This will present us with a input box which we will populate with the following value:
...
- Enabled the OIDC method
- Configure the session domain
- Define an OIDC provider called `google`
- Add the specific details for the Google OIDC flow and the credentials we obtained previously
...
then you can also supply these paramters as environment variables. All Flipt configuration
