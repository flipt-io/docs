...
You can configure Flipt with [environment variables](/configuration/overview#environment-variables) in the `fly.toml` file. For example, to configure Flipt to use a custom port and enable the DEBUG logging you can add the following to the `fly.toml` file:
