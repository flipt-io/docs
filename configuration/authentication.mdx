...
Flipt users/operators could have a use case of locking down access to the Flipt API to a certain group of users within their organization behind OIDC.
Since OIDC has the ability to get retrieve email on the server side, we provide a configuration option of using `email_matches` which are regexes that can be used to match against the OIDC email.
