...
There are a good amount of OIDC providers that support the PKCE (Proof Key for Code Exchange) flow as well as the implicit OAuth flow. Flipt allows for a piece of configuration to enable PKCE for all the legs of the OIDC authentication flow.

To enable this, you will need to set the [`use_pkce`](/configuration/overview#authentication-methods-oidc) property to `true` for each provider you would like to leverage PKCE with.
...
it's possible to secure Flipt simply by running it behind a reverse proxy in your own trusted environment.
