...
Login in the UI will require a session compatible authentication method (e.g. [OIDC](#OIDC)) be enabled.
</Warning>
...
Once enabled, all API routes will require a [client token](/authentication/overview#client-tokens) be presented in-order to authenticate the request.
...
See the [Authentication](/authentication/overview) documentation for more details on Flipt's API authentication handling.
...
Otherwise, the other properties are not required to be explicitly configured.

However, in order to best secure your instance of Flipt, we advise you run Flipt with `secure: true`.
This will require you expose Flipt over HTTPS.
Additionally, we advise you configure a `csrf.key` with a 32 or 64 byte random string of data.
...
Further explanation for using this method can be found in the [Authentication: Token](/authentication/methods#token) documentation section of this site.

#### Method: OIDC

<Note>
This is a `session compatible` authentication method.
</Note>

The `oidc` method provides the ability to establish client tokens via an OAuth 2.0 with OIDC flow.
Once enabled and configured appropriately, the UI will automatically leverage it and present any configured providers as login options.
...
> The above configuration would result in a login option in the UI for `some_provider`.

Multiple providers beneath the `providers` section of this configuration can be defined.
Each will result in a Login option being presented in the UI.
Along with a separate space being added in the API to support each providers flow.

Flipt has been experimented with each of the following providers:
...
Though the intention is that it should work with other OIDC providers.
These are just the handful the Flipt team has validated.
...
Using Google as an example and the documentation linked above; given we obtained the following credentials for a Google OAuth client:
...
We could create a provider defintion in our configuration like so:
...
<Info>
Additional, `scopes` are not 100% necessary. However, adding them will result in Flipt being able to identify more details about your users. Enabling us to present personalized greeting messages and user profile pictures in the UI.
</Info>
...
<Tip>
Google's consent screen can be configured to only accept accounts who are within your Google Workspace organization.

Other providers have similar mechanisms for attentuating who can leverage this authentication flow.
</Tip>
