...
The UI will require a session compatible authentication method (e.g. [OIDC](#OIDC)) be enabled.
</Warning>
...
In order to best secure your instance of Flipt, we that advise you run Flipt with `secure: true`.
...
<Info>
Additional `scopes` such as `profile` are not 100% necessary, however, adding them will result in Flipt being able to identify more details about your users such as personalized greeting messages and user profile pictures in the UI.
</Info>
