...
There is no known exhaustive list of environment variables that the [opentelemetry-go](https://github.com/open-telemetry/opentelemetry-go) respects according to the spec, but if you search around in the repository you can see if any particular environment variables are respected.
