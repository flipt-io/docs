...
However, the default settings have limitations, which might make it impractical in a production environment.
This guide will explore some Flipt deployment configurations for increased scalability and reliability.

## The Defaults

By default, Flipt can only run as a single instance process backed by SQLite. Additionally, all API interactions go directly to this database.
...
An externally hosted relational database is required to scale Flipt horizontally (run multiple instances with a shared backend). Postgres, MySQL and CockroachDB are the currently supported relational backends. Check out [Configuration: Storage](/configuration/storage) for more details on how to configure Flipt's available storage backends.
